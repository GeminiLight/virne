...
class NeuroSolver(Solver):
    ...
    def __init__(self, controller, recorder, counter, **kwargs):
        super(NeuroSolver, self).__init__(controller, recorder, counter, **kwargs)
        self.k = 2.5
        self.alpha = 10
        self.beta = 7.0
        self.gamma = 3.0
    ...
    def rank_edge(self, network):
        ...
        for i in range(network.num_nodes):
            for j in range(i, network.num_nodes):
                link_rank_matrix[i,j] = self.gamma * distances[i][j] / distance_max
                link_rank_matrix[j,i] = self.gamma * distances[j][i] / distance_max
