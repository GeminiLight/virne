"""
Modified main.py for running PL-Rank experiments on tree topology with clean switches.
...
4. Run the PL-Rank solver with the specified configuration

Usage:
    python main_tree_pl_rank.py p_net_setting=tree_p_net_setting \
...
@hydra.main(version_base=None, config_path="settings", config_name="main_tree_pl_rank")
def run(config: DictConfig):
    """
    Run the VNE simulation with tree topology and clean switches.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running PL-Rank solver on tree topology with clean switches (routing-only)")
