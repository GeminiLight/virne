"""
Modified main.py for running MCTS experiments on tree topology with clean switches.
...
4. Run the Monte Carlo Tree Search (MCTS) solver with the specified configuration

Usage:
    python main_tree_mcts.py p_net_setting=tree_p_net_setting \
...
@hydra.main(version_base=None, config_path="settings", config_name="main_tree_mcts")
def run(config: DictConfig):
    """
    Run the VNE simulation with tree topology and clean switches using MCTS.
    """
    print(f"\n{'-' * 20}    Start     {'-' * 20}\n")
    print("Running MCTS solver on tree topology with clean switches (routing-only)")
    print(f"Seed: {config.experiment.seed}")
    print(f"Solver: {config.solver.solver_name}")
    print(f"Computation budget: {config.solver.computation_budget}")
    print(f"Exploration constant: {config.solver.exploration_constant}")
